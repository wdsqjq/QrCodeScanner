...
import per.wsj.lib.qrcodescanner.decoding.CaptureActivityHandler;
import per.wsj.lib.qrcodescanner.decoding.InactivityTimer;
...
public class ScanFragment extends Fragment implements SurfaceHolder.Callback {
    ...
    public void handleDecode(Result result, Bitmap barcode) {
        ...
        if (result == null || TextUtils.isEmpty(result.getText())) {
            ...
            if (mScanCallback != null) {
                mScanCallback.onAnalyzeSuccess(barcode, result.getText());
            }
        }
        getActivity().finish();
    }
